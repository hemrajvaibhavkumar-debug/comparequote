  ...
  const validateForm = (): boolean => {
    ...
    for (let i = 0; i < sheets.length; i++) {
      ...
      if (!sheet.purpose.trim()) {
        showToast(`Please explain Responsibility for Sheet #${sheetNum}`, "error");
        setSelectedSheetIndex(i);
        return false;
      }
      if (!sheet.frequency) {
  ...
  const exportToExcel = async (data: ItAuditSubmission) => {
    try {
      ...
      data.sheets.forEach((sheet, idx) => {
        ...
        setKeyValueRow(ws, 7, 'Responsibility of Sheet', sheet.purpose);
        ...
        setKeyValueRow(ws, 16, 'Extra Notes', sheet.extra_notes || 'N/A');

        ws.getCell('A18').value = 'COLUMNS & CRUD ACTIONS';
        ws.getCell('A18').font = { name: 'Arial', size: 11, bold: true };

        const headers = ['Column Name', 'CRUD Actions', 'Description / Notes'];
        headers.forEach((h, colIndex) => {
          const cell = ws.getCell(19, colIndex + 1);
          cell.value = h;
          cell.font = { name: 'Arial', size: 10, bold: true, color: { argb: 'FFFFFFFF' } };
          cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: ORANGE_HEX } };
          cell.alignment = { horizontal: 'center', vertical: 'middle' };
  ...
  const exportAllToExcel = async () => {
    ...
    try {
      ...
      const sheetsHeaders = [
        ...
        'Sheet Name', 'Google Sheet Link', 'Responsibility of the Sheet', 
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
      ...
      {activeTab === 'form' ? (
        <div className="space-y-6">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-4 py-2.5 bg-slate-55 dark:bg-slate-950 border border-slate-200 dark:border-slate-805 rounded-xl text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-orange-500/20"
                  />
                </div>
              </div>
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <select
                    ...
                    className="w-full pl-9 pr-4 py-2.5 bg-slate-55 dark:bg-slate-950 border border-slate-200 dark:border-slate-805 rounded-xl text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-orange-500/20 text-slate-800 dark:text-slate-100"
                  >
          ...
          <div className="bg-slate-100/80 dark:bg-slate-950/40 p-2 rounded-2xl border border-slate-200/80 dark:border-slate-800/80 flex items-center justify-between gap-4 overflow-x-auto">
            <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
                ...
                <div 
                  ...
                  className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border transition-all duration-200 ${
                    selectedSheetIndex === index
                      ? 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 shadow-sm text-orange-600 font-extrabold ring-1 ring-orange-500/20'
                      : 'bg-transparent border-transparent text-slate-500 dark:text-slate-400 hover:bg-slate-200/50 dark:hover:bg-slate-900/50 hover:text-slate-800 dark:hover:text-slate-200'
                  }`}
                >
                  <FileSpreadsheet className={`w-3.5 h-3.5 shrink-0 ${selectedSheetIndex === index ? 'text-orange-500' : 'text-slate-400'}`} />
                  <button
                    type="button"
                  ...
                  {sheets.length > 1 && (
                    <button
                      ...
                      className="text-slate-400 hover:text-rose-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors p-0.5 rounded-md cursor-pointer ml-1"
                      title="Remove sheet"
                    >
                      <X className="w-3 h-3" />
                    </button>
              ...
              <button
                type="button"
                onClick={addSheet}
                className="w-8 h-8 flex items-center justify-center bg-white dark:bg-slate-900 hover:bg-orange-500 hover:text-white dark:hover:bg-orange-500 text-slate-500 border border-slate-200 dark:border-slate-800 hover:border-orange-500 rounded-xl transition-all shadow-sm hover:scale-105 active:scale-95 cursor-pointer shrink-0"
            ...
            <div className="text-[10px] font-black uppercase tracking-wider text-slate-400 mr-2 hidden sm:block">
              Total Audited: <span className="text-orange-500 font-extrabold">{sheets.length}</span> {sheets.length === 1 ? 'Sheet' : 'Sheets'}
            </div>
          </div>
              ...
              <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
                ...
                <div className="space-y-5">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <ExternalLink className="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" />
                      <input
                        type="url"
                        ...
                        className="w-full pl-9 pr-10 py-2.5 bg-slate-55 dark:bg-slate-950 border border-slate-200 dark:border-slate-850 rounded-xl text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-orange-500/20 text-blue-600 underline"
                      />
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Responsibility of the sheet *</label>
                    <div className="relative">
                      <ClipboardList className="w-4 h-4 text-slate-400 absolute left-3 top-3 pointer-events-none" />
                      <textarea
                        rows={3}
                        value={activeSheet.purpose}
                        onChange={e => updateSheetValue('purpose', e.target.value)}
                        placeholder="Describe what processes/actions this sheet is responsible for"
                        className="w-full pl-9 pr-4 py-2.5 bg-slate-55 dark:bg-slate-950 border border-slate-200 dark:border-slate-805 rounded-xl text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-orange-500/20 resize-none"
                      />
                    </div>
                    <p className="text-[9px] text-slate-400 font-semibold italic mt-0.5">Describe who is responsible for updating, maintaining, and using this spreadsheet.</p>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-6">
                      ...
                      {ROLES.map(role => {
                        ...
                        return (
                          <button
                            ...
                            className={`px-3 py-2.5 text-left rounded-xl border text-[10px] font-black uppercase tracking-tight transition-all duration-200 cursor-pointer flex items-center justify-between hover:scale-[1.015] hover:border-orange-400 active:scale-[0.985] ${
                              checked
                                ? 'bg-orange-50/50 dark:bg-orange-950/15 border-orange-400/80 text-orange-600 dark:text-orange-400 shadow-sm ring-1 ring-orange-500/10'
                                : 'bg-slate-50/50 dark:bg-slate-950/20 border-slate-200 dark:border-slate-805 text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-900/60'
                            }`}
                          >
                            <span>{role}</span>
                            {checked ? (
                              <CheckCircle2 className="w-4 h-4 text-orange-500 shrink-0 scale-110 transition-transform" />
                            ) : (
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="space-y-2">
                      {TIME_SPENT_OPTIONS.map(opt => {
                        const isSelected = activeSheet.time_spent === opt;
                        return (
                          <button
                            key={opt}
                            type="button"
                            onClick={() => updateSheetValue('time_spent', opt)}
                            className={`flex items-center justify-between w-full px-4 py-2.5 rounded-xl border text-xs font-semibold cursor-pointer transition-all duration-200 hover:scale-[1.01] hover:border-orange-400 active:scale-[0.99] text-left ${
                              isSelected
                                ? 'bg-orange-50/50 dark:bg-orange-950/15 border-orange-400/80 text-orange-600 dark:text-orange-400 shadow-sm ring-1 ring-orange-500/10'
                                : 'bg-slate-50/50 dark:bg-slate-950/20 border-slate-200 dark:border-slate-805 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-900/60'
                            }`}
                          >
                            <div className="flex items-center gap-3">
                            ...
                            {isSelected && (
                              <CheckCircle2 className="w-4 h-4 text-orange-500 shrink-0 scale-110" />
                            )}
                          </button>
                        );
                      })}
              ...
              <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
                <div className="flex items-center gap-2 mb-6 pb-3 border-b border-slate-100 dark:border-slate-850">
                  ...
                  <h3 className="text-sm font-black uppercase tracking-wider text-slate-850 dark:text-slate-200">6. Extra Notes</h3>
                </div>
                <div className="space-y-4">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Additional Remarks (Optional)</label>
                    <div className="relative">
                      <FileText className="w-4 h-4 text-slate-400 absolute left-3 top-3 pointer-events-none" />
                      <textarea
                        rows={3}
                        value={activeSheet.extra_notes || ''}
                        onChange={e => updateSheetValue('extra_notes', e.target.value)}
                        placeholder="Any extra comments, warnings, or operational details..."
                        className="w-full pl-9 pr-4 py-2.5 bg-slate-55 dark:bg-slate-950 border border-slate-200 dark:border-slate-805 rounded-xl text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-orange-500/20 resize-none text-slate-800 dark:text-slate-100"
                      />
                    </div>
                    <p className="text-[9px] text-slate-400 font-semibold italic">Provide any additional context or operational details not covered above.</p>
                  </div>
                </div>
              </div>
