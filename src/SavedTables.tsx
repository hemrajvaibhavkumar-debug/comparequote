  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-8 relative">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 text-sm text-indigo-700 bg-indigo-50 border border-indigo-100/50 px-4 py-2 rounded-xl shadow-xs shrink-0">
      ...
      <div className="bg-white p-4 rounded-2xl shadow-xs border border-slate-100 grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="relative md:col-span-2 group">
          ...
          <input 
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-slate-50/50 border border-slate-200 focus:border-indigo-500 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/10 text-slate-800 transition-all font-medium"
          />
        </div>
        
        <div className="relative group">
          <Factory className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 group-focus-within:text-indigo-600 transition-colors" />
          <select 
            value={filterPlant}
            onChange={e => setFilterPlant(e.target.value)}
            className="w-full pl-10 pr-4 py-2.5 bg-slate-50/50 border border-slate-200 focus:border-indigo-500 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/10 text-slate-800 transition-all font-medium appearance-none cursor-pointer"
          >
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="p-16 text-center space-y-4 max-w-md mx-auto">
            ...
            <button onClick={() => { setSearch(''); setFilterPlant(''); setFilterPreparedBy(''); }} className="text-indigo-600 text-xs font-bold hover:underline uppercase tracking-wider">Clear all filters</button>
          </div>
        ) : (
                ...
                {filteredTables.map(t => (
                  <tr key={t.id} className="group hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4.5">
                      <Link to={`/saved/${t.id}`} className="font-extrabold text-slate-900 group-hover:text-indigo-600 transition-colors flex items-center gap-2">
                        {t.doc_no}
                    ...
                    <td className="px-6 py-4.5">
                      <span className="px-3 py-1 bg-indigo-50 border border-indigo-100/50 text-indigo-700 rounded-xl text-xs font-semibold">
                    ...
                    <td className="px-6 py-4.5 text-right">
                      <div className="flex justify-end gap-2.5">
                        <Link to={`/saved/${t.id}`} className="p-2 hover:bg-indigo-50 text-slate-400 hover:text-indigo-600 border border-transparent hover:border-indigo-100 rounded-xl transition-all shadow-none hover:shadow-xs" title="View Table">
                          <Eye className="w-4 h-4" />
                        </Link>
                        <button onClick={(e) => deleteTable(t.id, e)} className="p-2 hover:bg-rose-50 text-slate-400 hover:text-rose-600 border border-transparent hover:border-rose-100 rounded-xl transition-all shadow-none hover:shadow-xs" title="Delete">
