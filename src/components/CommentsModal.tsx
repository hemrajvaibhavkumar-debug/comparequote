...
export default function CommentsModal({ isOpen, onClose, comments, onAddComment, onUpdateComment, title }: CommentsModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-2xl overflow-hidden flex flex-col h-[75vh] max-h-[600px] animate-in zoom-in-95 duration-200">
        ...
        <div className="p-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-md font-black text-slate-900 uppercase tracking-tight">Internal Points & Observations</h3>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{title}</p>
            </div>
          </div>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-600 transition-colors p-1.5 hover:bg-slate-100 rounded-full cursor-pointer">
            <X className="w-5 h-5" />
          </button>
        </div>

        {/* Notepad Textarea */}
        <div className="flex-1 p-6 bg-amber-50/20 relative flex flex-col overflow-hidden">
          ...
          <textarea
            ...
            className={`w-full flex-1 p-5 rounded-xl border border-amber-200/60 bg-amber-50/30 text-slate-800 text-sm font-medium leading-[28px] focus:outline-none transition-all resize-none shadow-inner custom-scrollbar ${
              canEdit ? 'focus:ring-2 focus:ring-amber-500/10 focus:border-amber-400' : 'cursor-not-allowed bg-slate-50/50 border-slate-200'
            }`}
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={onClose}
              className="px-5 py-2 border border-slate-200 text-slate-650 bg-white hover:bg-slate-50 rounded-xl text-xs font-bold uppercase tracking-widest cursor-pointer transition-all active:scale-[0.98]"
            >
              Close
            </button>
            {canEdit ? (
              <button
                onClick={handleSave}
                disabled={isSaving}
                className="px-6 py-2 bg-slate-900 text-white rounded-xl text-xs font-bold uppercase tracking-widest hover:bg-black transition-all shadow-md shadow-slate-100 flex items-center gap-2 cursor-pointer disabled:opacity-40 disabled:scale-100 active:scale-[0.98]"
              >
                {isSaving ? (
              ...
              <div className="flex items-center gap-1 text-[10px] font-black text-rose-500 uppercase tracking-widest px-4 py-2 bg-rose-50 rounded-xl border border-rose-100">
